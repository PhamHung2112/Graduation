  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const res = await api.get(
        `product/all?limit=8${filterOption?.typeId ? `&&typeId=${filterOption?.typeId}` : ''}${
        ...
        }${filterOption?.sizes?.length ? `&&sizeId=${filterOption?.sizes.join(',')}` : ''}`
      );
    ...
    if (filterOption?.typeId) getData();
  }, [filterOption]);
